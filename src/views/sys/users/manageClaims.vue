<template>
  <BasicDrawer @register="registerDrawer" title="管理声明" width="500px">
    <div>
      ...
      <div v-if="state.dataList.length > 0">
        ...
        >
          ...
          <a-col :span="8">
            <TableAction
              style="margin-left: 30px; line-height: 30px"
              :actions="[
                {
                  label: '修改',
    ...
    <EditClaimsModel
      @register="registerEditClaimsModel"
      @reloadTable="reloadTable"
      :bodyStyle="{ 'padding-top': '0' }"
    />
    <CreateClaimsModel
      @register="registerCreateClaimsModel"
      @reloadTable="reloadTable"
      :bodyStyle="{ 'padding-top': '0' }"
    />
  </BasicDrawer>
...
<script lang="ts">
  import { defineComponent, reactive, defineEmit } from 'vue';
  import { BasicDrawer, useDrawerInner } from '/@/components/Drawer';
  import EditClaimsModel from './editclaimsModel.vue';
  import CreateClaimsModel from './createclaimsModel.vue';
  import { getUserClaimsAsync, deleteUserClaimsAsync } from './index';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const [registerDrawer] = useDrawerInner((data) => {
        ...
        getUserClaimsAsync(data.record.id).then((res) => {
      ...
      const handleDelete = async (record: Recordable) => {
        deleteUserClaimsAsync(state.record.id, record).then(() => {
          reloadTable();
        });
      };

      const reloadTable = async () => {
        getUserClaimsAsync(state.record.id).then((res) => {
          state.dataList = res.items;
        });
      };
